...
import 'package:msm_mobile_app/models/airPortResponse.dart';
...
Future<List<Map<String, dynamic>>> airPortFetchData(destination) async {
 
  var url =
      'https://www.mv-tel.com/MSM_Viaticos_API_Test/api/Aeropuerto';
  ...
  if (response.statusCode == 200) {
    final body = jsonDecode(response.body);
    final responseData = AirPortsList.fromJson(body);
    List<Map<String, dynamic>> myList = [];
    for (var i in responseData.airports) {
      myList.add({'value': '${i.nombre}', 'label': '${i.nombre}'},);
    }

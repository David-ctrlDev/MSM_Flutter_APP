...
class _FormContainer3 extends State<FormContainer3> {
    ...
    setState(() {
      ...
      if (destinationValue == 'nacional') {
        customField = SelectFormField(
          ...
          items: airPortsList,
          onChanged: (val) => {_valueChangedAirport = val},
        );
      } else {
    ...
    return showDialog<void>(
        ...
        builder: (
          ...
          return AlertDialog(
            ...
            title: const Text(
              'Desplazamientos Aeroportuarios',
  ...
  List<DataRow> getRows(List dataTableRows) => dataTableRows
      .map((item) => DataRow(
              selected: selectedAirport.contains(item),
              onSelectChanged: (isSelected) => setState(() {
                    final isAdding = isSelected != null && isSelected;
                    isAdding
                        ? selectedAirport.add(item)
                        : selectedAirport.remove(item);

                    for (var i in selectedAirport) {
                      airPortsListAdd
                          .removeWhere((item) => item['id'] == i['id']);
                      airPortsListAdd.remove(i);
                      dataTableRows.remove(i);

                      setState(() {
                        airPortsListAdd = airPortsListAdd;
              ...
              cells: [
                DataCell(Text(item['flyNumber']), onTap: () => null),
                DataCell(Text(item['airport']), onTap: () => null),
                DataCell((item['icon']), onTap: () => null),
